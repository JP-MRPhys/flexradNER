from backend.app import Flask, request, jsonify
...
quickumls_path = 'umls_index/'   #add to config 
quickumls_path = getulmsdir()
# Configure logging
...
def analyze_medical_report():
    ...
    try:
        ...
        # Analyze the report (Assume it returns two outputs)
        output_1, output_2 = medical_analyzer.analyze_report(data['text'])

        # Create a structured JSON response with both outputs
        response = {
            "output_1": output_1,
            "output_2": output_2
        }

        # Return the JSON response with a successful 200 status code
