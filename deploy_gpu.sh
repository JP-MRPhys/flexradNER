...
INSTANCE_TYPE="g4dn.xlarge" # GPU instance for ML tasks
AMI_ID="ami-0c02fb55956c7d316" # Amazon Linux 2 AMI (adjust for your region)
...
# Launch a GPU-enabled EC2 instance
...
ssh -o "StrictHostKeyChecking no" -i "${EC2_KEY_PAIR_NAME}.pem" ec2-user@$PUBLIC_IP << EOF
    ...
    # Install NVIDIA drivers and dependencies for GPU
    echo "Installing NVIDIA drivers..."
    sudo yum install -y gcc kernel-devel-$(uname -r)
    curl -s -O https://us.download.nvidia.com/tesla/515.76/NVIDIA-Linux-x86_64-515.76.run
    sudo bash NVIDIA-Linux-x86_64-515.76.run --silent
